...
const getFailedSearches = async (req, res) => {
    try {
        ...
        const failedSearches = await Prompt.find({
            $or: [
        ...
        const totalFailed = await Prompt.countDocuments({
            $or: [
                { views: 0 },
                { views: { $exists: false } },
                { views: null }
...
const getDatabasePerformance = async (req, res) => {
    try {
        ...
        const totalDocuments = await Prompt.countDocuments();
        ...
        await Prompt.findOne();   // Run a simple query
...
const getCacheHitRate = async (req, res) => {
    try {
        ...
        const totalConcepts = await Prompt.countDocuments();

        // Count concepts that have been viewed at least once ("cache hits")
        const viewedConcepts = await Prompt.countDocuments({ views: { $gt: 0 } });
